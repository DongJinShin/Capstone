...
public class LogInPresenter implements LogInPresenterInterface
{
    ...
    @Override
    public void logIn(String ID, String password, final int userType)
    {
        ...
        Call<UserModel> productModel = retrofitinterface.logIn(ID, securityPassword, userType);
        productModel.enqueue(new Callback<UserModel>()
        {
            ...
            public void onResponse(Call<UserModel> call, Response<UserModel> response)
            {
                if(userType == 0)
                {
                    logInView.goToManagerMainActivity(response.body().getID());
                }
                else
                {
                    logInView.goToUserMainActivity(response.body().getID());
                }
            }
